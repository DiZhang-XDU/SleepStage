...
def edf_header(f):
  ...
  # recording info)
  h['local_subject_id'] = f.read(80).strip().decode()
  h['local_recording_id'] = f.read(80).strip().decode()

  # parse timestamp
  (day, month, year) = [int(x) for x in re.findall('(\d+)', f.read(8).decode())]
  (hour, minute, sec)= [int(x) for x in re.findall('(\d+)', f.read(8).decode())]
  h['date_time'] = str(datetime.datetime(year + 2000, month, day,
    hour, minute, sec))

  # misc
  header_nbytes = int(f.read(8).decode())
  subtype = f.read(44).decode()[:5]
  h['EDF+'] = subtype in ['EDF+C', 'EDF+D']
  h['contiguous'] = subtype != 'EDF+D'
  h['n_records'] = int(f.read(8).decode())
  h['record_length'] = float(f.read(8).decode())  # in seconds
  nchannels = h['n_channels'] = int(f.read(4).decode())

  # read channel info
  channels = range(h['n_channels'])
  h['label'] = [f.read(16).strip().decode() for n in channels]
  h['transducer_type'] = [f.read(80).strip().decode() for n in channels]
  h['units'] = [f.read(8).strip().decode() for n in channels]
  h['physical_min'] = np.asarray([float(f.read(8).decode()) for n in channels])
  h['physical_max'] = np.asarray([float(f.read(8).decode()) for n in channels])
  h['digital_min'] = np.asarray([float(f.read(8).decode()) for n in channels])
  h['digital_max'] = np.asarray([float(f.read(8).decode()) for n in channels])
  h['prefiltering'] = [f.read(80).strip().decode() for n in channels]
  h['n_samples_per_record'] = [int(f.read(8).decode()) for n in channels]
  f.read(32 * nchannels).decode()  # reserved
...
class BaseEDFReader:
  ...
  def read_raw_record(self):
    ...
    for nsamp in self.header['n_samples_per_record']:
      samples = self.file.read(nsamp * 2).decode()
